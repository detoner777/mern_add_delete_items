...
class Item extends Component {
  ...
  componentDidMount = () => {
    fetch("http://localhost:8000/item/" + this.props.match.params.id)
      .then(res => {
        return res.json();
      })
      .then(blob => {
  ...
  deleteItem = () => {
    ...
    if (confirmDelete) {
      ...
      fetch(
        "http://localhost:8000/item/" + this.props.match.params.id,
        options
      ).then(res => {
        console.log(res);
        ...
        <section>
          ...
          <div className="itemCreation">
            ...
            <div className="preview">
              ...
              <p>
                Product Name: <strong> {this.state.name}</strong>
              </p>
              <p>
                Product Description: <strong> {this.state.description}</strong>{" "}
              </p>
              <p>
                Product Price: <strong> {this.state.price} </strong>
              </p>
